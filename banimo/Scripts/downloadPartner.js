...
var uploadImage = function (imageParent, imageName, input) {
    ...
    if (input.files && input.files[0]) {
        ...
        var htmldiv = `<div class="imageCover" style="postion:relative !important; width:150px;margin:5px"><div style="position:relative"><img style="width:150px" id="` + processID + `" src="#" /> </div></div> `;
...
var sendFile = function (key, value, imageName) {
    ...
    request.open('POST', '/Partner/sendToServerByJS');
...
var removeImageFromItem = function (DAT, imageName) {
    let id =  DAT.split('***')[0];
    let name = DAT.split('***')[1];

    $.ajax({
        url: "/Partner/deleteimage",
        data: {
            id: id,
            title: name
        },
